  ...
  useEffect(() => {
    const calculateTotalWidth = () => {
      if (carouselRef.current && images.length > 0) {
        const oneImageElement = carouselRef.current.querySelector(
          `.${styles.oneImageCarousel}`
        );
        if (oneImageElement) {
          const oneImageWidth = oneImageElement.offsetWidth;
          const gap =
            parseFloat(getComputedStyle(carouselRef.current).gap) || 0;
          const numUniqueImages = images.length / 2; // Ya que duplicamos las imágenes
          const totalWidth =
            numUniqueImages * oneImageWidth + (numUniqueImages - 1) * gap;
          document.documentElement.style.setProperty(
            "--total-width",
            `${totalWidth}px`
          );
        ...
        <div className={styles.carouselWrapper}>
          <div className={styles.imageCarousel} ref={carouselRef}>
            {images.map((image, index) => (
              <div
                className={styles.oneImageCarousel}
                key={`${image.id}-${index}`}
              >
